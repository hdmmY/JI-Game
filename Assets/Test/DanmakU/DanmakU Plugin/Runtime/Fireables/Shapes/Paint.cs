...
namespace DanmakU.Fireables
{
    ...
    public class Paint : Fireable
    {
        ...
        {
            var emitter = new GameObject ("PaintEmitter").AddComponent<PaintEmitter> ();
            emitter.SetEmitParams (PaintData, CenterAngle, ShiftAngle, Delay);
            emitter.Emit (config, Child);
        }

        protected class PaintEmitter : MonoBehaviour
        {
